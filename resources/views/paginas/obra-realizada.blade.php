    ...
    <div class="container">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb text-uppercase py-2 font-size-15 m-0">
                ...
                <li class="breadcrumb-item fst-italic" aria-current="page" style="color: black;">
                    <a href="{{ route('obras-realizadas') }}" class="text-decoration-none" style="color: black;">Obras Realizadas</a>
                </li>
                <li class="breadcrumb-item active fst-italic" aria-current="page" style="color: black;">{{ $wm->content_1 }}</li>
            </ol>
        </nav>
    </div>
</div>
    ...
    <div class="container">
        <div class="row">
            <aside class="col-sm-12 col-md-3 d-sm-none d-md-block">
                <ul class="p-0" style="list-style: none;">
                    @foreach ($worksMade as $m)
                        <li class="py-2  @if($m->id == $wm->id) active @endif"> 
                            <a href="{{ route('obra-realizada', ['id'=> $m->id]) }}" class="toggle d-block p-2 text-decoration-none  text-decoration-none text-dark">{{$m->content_1}}</a>
                        </li>               
                    @endforeach                      
                </ul>             
            </aside>
            <div class="col-sm-12 col-md-9">
                <div class="mb-4">
                    @if (count($wm->images))
                        @if (Storage::disk('public')->url($wm->images()->first()->image))
                            <img src="{{ asset($wm->images()->first()->image) }}" class="img-fluid w-100" id="imgcurrent">
                        @else
                ...
                <div class="mb-sm-2 mb-md-5">
                    <h3 class="text-primary mb-3 font-size-17 fw-bold"> {{ $wm->content_1 }}</h3>
                    <div class="font-size-14">{!! $wm->content_2 !!}</div>
                </div>
                <div class="d-flex justify-content-sm-center justify-content-md-start flex-wrap mb-sm-2 mb-md-5">
                    @if($wm->content_6)
                        <a href="{{ route('ficha-tecnica', ['id'=> $wm->id]) }}" class="me-sm-0 me-md-3 mb-sm-3 mb-md-0 px-3  btn ficha-tecnica rounded-pill font-size-15 w-sm-100 w-md-50 position-relative text-center" style="width: 270px;">
